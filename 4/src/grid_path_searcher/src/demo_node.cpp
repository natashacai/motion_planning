...
TrajectoryStatePtr *** TraLibrary;  //[i][j][k] 3D
...
{//
    Vector3d acc_input;  //u(t) input of the dynamic system
    Vector3d pos,vel;
    int a =0 ;
    int b =0 ;
    int c =0 ;
    ...
    TraLibrary  = new TrajectoryStatePtr ** [_discretize_step + 1];     //recored all trajectories after input, 3rd pointer array
            ...
            for(int k=0; k <= _discretize_step; k++){   //acc_input_az
                ...
                for(int step=0 ; step<=_time_step ; step ++){
                    ...
                    */
                    ...
                    vel(0) = vel(0) + delta_time * acc_input(0);
                    vel(1) = vel(1) + delta_time * acc_input(1);
                    vel(2) = vel(2) + delta_time * acc_input(2);

                    //S(t+1) = S(t) + V(t)*delta_t + 0.5 * (delta_t)^2 * a(t) 
                    pos(0) = pos(0) + vel(0)*delta_time + 0.5*pow(delta_time, 2)*acc_input(0);
                    pos(1) = pos(1) + vel(1)*delta_time + 0.5*pow(delta_time, 2)*acc_input(1);
                    pos(2) = pos(2) + vel(2)*delta_time + 0.5*pow(delta_time, 2)*acc_input(2);
