...
{   
    ...
    {    
        ...
        // set obstalces into grid map for path planning  将障碍物信息设置进入栅格化地图，为后续路径规划做准备 
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    //进行JPS 路径规划编写时，将_use_jps 的值置为1 即可
...
#if _use_jps
    {
        //Call JPS to search for a path 使用JPS 进行路径搜索  
        ...
        //Visualize the result 可视化路径
        visGridPath   (grid_path, _use_jps);
        visVisitedNode(visited_nodes);

        //Reset map for next call 重置地图
...
int main(int argc, char** argv)
{
    ...
    //定义了结构体AstarPathFinder 变量_astar_path_finder，该结构体存储、实现了Astar 路径规划所需的所有信息和功能 
    _astar_path_finder  = new AstarPathFinder();
    _astar_path_finder  -> initGridMap(_resolution, _map_lower, _map_upper, _max_x_id, _max_y_id, _max_z_id);
    //定义了结构体JPSPathFinder 变量_jps_path_finder，该结构体存储、实现了JPS 路径规划所需的所有信息和功能
