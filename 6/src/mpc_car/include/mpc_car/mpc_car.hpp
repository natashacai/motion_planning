...
class MpcCar {
 private:
  ...
  double ll_; // length of car?
  double dt_; //
  double rho_; //
  int N_; //
  double rhoN_; //

  double v_max_, a_max_, delta_max_, ddelta_max_; //constraints
  double delay_;

  arc_spline::ArcSpline s_; //spline of the trajectory
  double desired_v_;  //v reference

  osqp::IOSQP qpSolver_;  // solver for the quadratic programming
  ...
  void linearization(const double& phi,
                     ...
                     const double& delta) {
    // x_{k+1} = Ad * x_{k} + Bd * u_k + gd
    // phi, v, delta: linearization operating point
    ...
    Ad_(1,2) = v * cos(phi);
    Ad_(1,3) = sin(phi);
    Ad_(2,3) = tan(delta)/ll_;
    */
    Ad_ <<  0, 0, -v*sin(phi), cos(phi),
            0, 0,  v*cos(phi), sin(phi),
            0, 0, 0, tan(delta) / ll_,
            0, 0, 0, 0;
    Ad_ = Eigen::MatrixXd::Identity(n,n) + dt_* Ad_;

    /*
    Bd_.setZero();
    Bd_(3,0) = 1;
    Bd_(2,1) = v/(ll_ * pow(cos(delta),2));
    Bd_ = dt_ * Bd_;
    */
    Bd_ << 0, 0,
            0, 0,
    ...
    /*
    gd_.setZero();
    gd_(0,0) = v * phi * sin(phi);
    gd_(1,0) = -v * phi * cos(phi);
    gd_(1,0) = -(v * delta)/(ll_ * pow(cos(delta),2));
    gd_ = dt_ * gd_;
    */
